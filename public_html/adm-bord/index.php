...
$APPLICATION->SetTitle("Заявки по Объявлениям");
?>
<?$byNew = $USER->GetID();
...
<?if ($arUserNew["UF_AD_APP"] == "4") {?>
            ...
            if ($fill == "fill=1" || $fill == "") {
                ...
                if (isset($parID)) {
                    foreach ($parID as $NewParID) {
                    	$res = CIBlockElement::GetProperty(10, $NewParID, array("sort" => "asc", ), Array('ID' => 62));
                    ...
                    foreach ($parID as $NewParID) {
                        $res = CIBlockElement::GetProperty(10, $NewParID, array("sort" => "asc", ), Array('ID' => 104));
                    	while ($ob = $res->GetNext()) {
                    		$prop = $ob;
                    		if ($prop["ID"] == 104){
    ...
    <div class="panel-main-adm-ren">
        <div class="panel-main-adm-ren-top-left">
            <a class="not-verified" <?if ($styleInfo == 1) {?>style="font-weight: 700;"<?}?> href="/adm-bord/index.php?fill=1">Не проверенные</a> /
            <a class="yes-verified" <?if ($styleInfo == 2) {?>style="font-weight: 700;"<?}?> href="/adm-bord/index.php?fill=2">Принятые</a> /
            <a class="Rejected" <?if ($styleInfo == 3) {?>style="font-weight: 700;"<?}?> href="/adm-bord/index.php?fill=3">Отклонённые</a>
        </div>
        <div class="panel-main-adm-ren-top-left-t">
            <div class="info-panel-v">Объявления</div>
	...
	"new-castom-section-martimer-board-for-adm", 
	array(
...
<script>
    ...
    function general_all_adm(e) {
        ...
        } else if (type == "bor" && view == "delBe") {
            var overjs = "";
            var typeJS = type;
            var pubIDJS = pubID;
            var viewJS = view;
            AddGeneral(typeJS, pubIDJS, viewJS, overjs);
        } else if (type == "bor" && view == "editBe") {
            var overjs = document.querySelector('.form-titlle-fall-input').value;
            if (overjs != "") {
                var typeJS = type;
                var pubIDJS = pubID;
