...
<table cellpadding="0" cellspacing="0" border="0" width="100%">
		...
		<tr>
		<?endif;?>
		<td valign="top" width="<?=round(100/$arParams["LINE_ELEMENT_COUNT"])?>%" class="top-vack-card" id="<?=$this->GetEditAreaId($arElement['ID']);?>">
			<table cellpadding="0" cellspacing="2" border="0">
				<tr>
					...
					<?endif?>
					<td valign="top" class="top-left">
						<?if(!$arParams["TREE_DETAIL_PAGE_URL"]):?>
							<a href="<?=$arElement["DETAIL_PAGE_URL"]?>">
						<?endif;?>
						<div class="board-title" title="<?=$arElement["NAME"]?>"><?=$arElement["NAME"]?></div>
						<?if(!$arParams["TREE_DETAIL_PAGE_URL"]):?>
							</a>
						<?endif;?>
						<?
						...
						<?foreach($arElement["DISPLAY_PROPERTIES"] as $pid=>$arProperty):
						    //echo "<pre>";
						    //print_r($arProperty["ID"]);
						    //echo "<pre>";
						    if($arProperty["ID"] == 16) { 
						         echo '<span class="properties" id="properties_' .$arProperty["ID"]. '">'.$arProperty["NAME"];
                                
    							if(is_array($arProperty["DISPLAY_VALUE"])) {
    								echo '<span id="properties_bott_'. $arProperty["ID"] .'">';
    								echo implode("&nbsp;–&nbsp;", $arProperty["DISPLAY_VALUE"]); echo ' ' ; foreach($arElement["DISPLAY_PROPERTIES"] as $pid=>$arProperty): if($arProperty["ID"] == 51){ echo $arProperty["DISPLAY_VALUE"]; } endforeach; 
    								echo '</span>';
    							} else {
    								echo '<span id="properties_bott_'. $arProperty["ID"] .'">' . $arProperty["DISPLAY_VALUE"] . '</span>&nbsp';
    							}
    							echo '</span>';
						     } else if($arProperty["ID"] == 51 || $arProperty["ID"] == 52){
						         
						     } else if($arProperty["ID"] == 17) { 
						         echo '<span class="properties" id="properties_' .$arProperty["ID"]. '">'.$arProperty["NAME"];
    							...
    							if(is_array($arProperty["DISPLAY_VALUE"])) {
    								...
    								echo '<span id="properties_bott_'. $arProperty["ID"] .'">' . strtolower(FormatDate("d F Y", MakeTimeStamp($arProperty["DISPLAY_VALUE"]))); echo ' '; foreach($arElement["DISPLAY_PROPERTIES"] as $pid=>$arProperty): if($arProperty["ID"] == 52){ echo $arProperty["DISPLAY_VALUE"]; } endforeach; 
    								echo '</span>&nbsp';
    							}
    							echo '</span>';
						     } else {
						...
						<?endif;?>
						<?if ($pub_date)
							echo '<span class="board-date">Размещено<h1 class="border-data-data">'. strtolower(FormatDate("d F Y", MakeTimeStamp($pub_date))) . '</h1>' .'</span><br />';
						?>
					</td>
					<td valign="top" class="top-right">
					    <div class="top-right-comp-logo">
					        <?$create_user=$arElement["CREATED_BY"];
						...
						<div class="top-right-comp-title"><?echo $arUserT['WORK_COMPANY']?></div>
					</td>
				</tr>
			</table>
