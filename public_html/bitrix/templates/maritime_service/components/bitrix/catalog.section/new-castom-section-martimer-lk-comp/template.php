...
<div class="catalog-section bx-<?=$arParams['TEMPLATE_THEME']?>" data-entity="<?=$containerName?>">
	<?
	 //echo "<pre>";
	 //print_r($arResult['ITEMS'][2]['ACTIVE']);echo "<pre>";
							...
							<div class="row">
								...
								{    
								   //echo "<pre>"; print_r($item);echo "<pre>";
									?>
								<?
    								//Запрос текущго пользователя
    								$rsUserCurrent = CUser::GetByID($USER->GetID()); 
                                    $arUserCurrent = $rsUserCurrent->Fetch();
                                    
                                    //Запрос пользователя, который создал публикацию
                                    $create_user=$item["CREATED_BY"];
                                    $GLOBALS['users'] = array("CREATED_BY" => $create_user); 
                                    $userid = $create_user;
                                    $rsUser = CUser::GetByID($userid); 
                                    $arUserT = $rsUser->Fetch();
                                    //print_r ($arUserCurrent['ID']);
                                    
                                    //сравнение
                                    if ($arUserT['ID'] == $arUserCurrent['ID']) {
                                    ?>
    									<div class="col-xs-6 product-item-big-card">
    										<div class="row">
    											<div class="col-md-12">
    												...
    												$APPLICATION->IncludeComponent(
    													'bitrix:catalog.item',
    													'new-castom-catalog-item-martimer-lk-comp',
    													array(
    														'RESULT' => array(
    															'ITEM' => $item,
    															'AREA_ID' => $areaIds[$item['ID']],
    										...
    										<a class="close-lk" onclick="popup_show_lk(this);" data-popup="delete" data-id="<?=$item['ID']?>">
    										    <svg class="close-lk-svg" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                                                <path d="M18.9999 18.9999L10 10M10 10L1 1M10 10L19 1M10 10L1 19" stroke="#333333" stroke-opacity="0.5" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                                                </svg>
                                            </a>
    									</div>
    									<?
