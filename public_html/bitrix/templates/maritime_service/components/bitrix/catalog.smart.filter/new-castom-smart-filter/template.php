			...
			<div class="row">
				...
				foreach($arResult["ITEMS"] as $key=>$arItem)
				{
					...
					<div class="<?if ($arParams["FILTER_VIEW_MODE"] == "HORIZONTAL"):?>col-sm-6 col-md-4<?else:?>col-lg-12<?endif?> bx-filter-parameters-box <?if ($arItem["DISPLAY_EXPANDED"]== "Y"):?>bx-active<?endif?>"<?if($key == 51):?>style="display:none"<?endif?>>
						...
						<div <?if( $key == 16 || $key == 15){?> id="bx-filter-block_<?=$key?>" <?}?> class="bx-filter-block" data-role="bx_filter_block">
						  <?if($key == "15"){?>
...
//seach for block vac
    document.getElementById('visual_').onkeyup = function () {
        ...
        let elasticItems = document.querySelectorAll('#bx-filter-block_15 .bx-filter-input-checkbox');
        if (val != '') {
    ...
    */
    ...
    document.addEventListener("click", function(e) {
        $(document).ready(function() {
            ...
            let remselect = document.querySelector('.bx-filter-select-popup');
            let newPASD = document.querySelector('.product-item-image-slider-slide-container.slide');

            if (g == 0) {
                newPASD.innerHTML = remselect.innerHTML;
                var dropDown = $('<ul>', {
                        className: 'bx-filter-select-popup-list'
                    });
                
                select.each(function(i) {
                    var newSelectSafe = select.innerHTML;
                ...
                remselect.innerHTML = newPASD.innerHTML;
                newPASD.innerHTM = "";
                g++;
            }
        });
    ...
    $(document).ready(function() {
        document.addEventListener("click", function(e) {
            ...
            if (event.target.matches('.bx-filter-by-list-solid svg')){
                ...
                smartFilter.click(document.getElementById("arrFilter_16_MIN"))
                smartFilter.click(document.getElementById("arrFilter_16_MAX"))
            }
        })
    })
