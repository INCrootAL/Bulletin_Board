							...
							<div class="product-item-info-container product-item-hidden" data-entity="props-block">
								<dl class="product-item-properties">
									...
									{?>
										...
										<?if ($displayProperty['ID'] == 63) {?> 
    										<dt<?=(!isset($item['PROPERTY_CODE_MOBILE'][$code]) ? ' class="hidden-xs"' : '')?>>
    											ФИО
    										</dt>
										<?} else if ( $displayProperty['ID'] == 103  || $displayProperty['ID'] == 102 || $displayProperty['ID'] == 105){?>
										<?} else {?>
										    <dt<?=(!isset($item['PROPERTY_CODE_MOBILE'][$code]) ? ' class="hidden-xs"' : '')?>>
										...
										<?}?>
										<?if ($displayProperty['ID'] == 63) {?>
										<dd style = "margin-bottom: 12px;"<?=(!isset($item['PROPERTY_CODE_MOBILE'][$code]) ? ' class="hidden-xs"' : '')?>>
										...
										<?}?>
									<?}?>
									    <?//echo "<pre>";print_r ($item['DISPLAY_PROPERTIES']['R_VERIFIED_ADMINISTRATOR']['VALUE']); echo "<pre>";?>
									<?if ($item['DISPLAY_PROPERTIES']['R_R_DEVIATION']['VALUE'] != "" && $item['DISPLAY_PROPERTIES']['R_VERIFIED_ADMINISTRATOR']['VALUE'] == "Отклонено") {?> 
										    <div class="hidden-warning-not-publick"> <dd class="hidden-warning-not-publick-txt">Отклонено</dd><dd class="hidden-warning-not-publick-pre" onclick="popup_show_lk(this);" data-popup="deviation" data-info="<?=$item['DISPLAY_PROPERTIES']['R_R_DEVIATION']['VALUE']?>">Посмотреть причину отклонения</dd></div>
										<?} else if ($item['DISPLAY_PROPERTIES']['R_VERIFIED_ADMINISTRATOR']['VALUE'] == "Нет" && $item['DISPLAY_PROPERTIES']['R_DATE_PUBLICK']['VALUE'] == "" || 
										$item['DISPLAY_PROPERTIES']['R_DATE_PUBLICK']['VALUE'] == "Продление публикации"){?>
										    <div class="hidden-warning-not-publick"> <dd class="hidden-warning-not-publick-txt-adm">На проверке</dd></div>
										<?} else if ($item['DISPLAY_PROPERTIES']['R_VERIFIED_ADMINISTRATOR']['VALUE'] == "Нет" && $item['DISPLAY_PROPERTIES']['R_DATE_PUBLICK']['VALUE'] == "Срок действия вышел") {?>
										    <div class="hidden-warning-not-publick"> <dd class="hidden-warning-not-publick-txt-adm-term">Срок публикации вышел</dd><dd onclick="repeadApp(this)" class="hidden-warning-not-publick-pre-trem" data-type-pub="res" data-id-pub="<?=$item['ID']?>">Отправить повторно</dd></div>
										<?} else {?>
								        	<div style = "margin-top: 22px;" class="hidden-xs-time-publick">Размещено <dd class="hidden-xs-time-publick-valu"><? echo strtolower(FormatDate("d F Y", MakeTimeStamp($pub_date)))?></dd></div>
									    <?}?>
								</dl>
								<dl class="product-item-properties-comp-vac">
								     <div class="top-right-comp-logo">
								         
            					        <?$create_user=$item["CREATED_BY"];
								...
								<?if($USER->IsAuthorized()) {
								    ...
								    <dl class="product-item-properties-comp-favorites"><div data-type="<?=$arUserNa["UF_TYPE"];?>" <?if(!$USER->IsAuthorized()){ ?> onclick="popup_show(this);" data-popup="auth" <?}?>  class="properties-comp-favorites <?foreach ($favorites as $code => $disp){if ($disp == $item["ID"]){echo "active";} else {echo "";}
                                }?>" id="properties-comp-favorites-resume" data-item="<?echo $item["ID"]?>"></div></dl>
                                <?}?>
							</div>
							<?
						}
